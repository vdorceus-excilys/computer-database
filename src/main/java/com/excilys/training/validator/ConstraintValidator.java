...
public class ConstraintValidator implements Validator{
	...
	private void evaluate(EvalBox evalBox) throws FailedValidationException{		
	...
	private void lowerThan(EvalBox evalBox) throws FailedValidationException{
		String target = evalBox.constraint.lowerThan();		
		if(target.isEmpty()) 
			return;
		if(!refs.containsKey(target)) {
			throw new FailedValidationException("No target found to compare to");
		}				
		if(evalBox.constraint.clazz().equals("java.util.Date")) {
			...
			if(afterDate.before(beforeDate))//
				throw new FailedValidationException("The value of "+evalBox.fieldName+" isn't after the date in ::"+target);
			//completed without issues
		}		
	}
	private void greaterThan(EvalBox evalBox) throws FailedValidationException{
	...
	private void matches(EvalBox evalBox) throws FailedValidationException{
		...
		if(!regex.isEmpty()) {
			...
			if(!matcher.find())
				throw new FailedValidationException("The value of "+evalBox.fieldName+" doesn't match the regex condition :: "+regex);
		}
	}
	private void inList(EvalBox evalBox) throws FailedValidationException{
		String list = evalBox.constraint.inList().trim();		
		if(!list.isEmpty()) {
			Set<String> acceptedValues = new HashSet<>(Arrays.asList(list.split(",")));
			String value  = evalBox.content.toString();
			if(!acceptedValues.contains(value))
				throw new FailedValidationException("The value of "+evalBox.fieldName+" is not in the list of accepted values");
		}
	}
	private void maxSize(EvalBox evalBox) throws FailedValidationException{
		String value = (String)  evalBox.content;
		int maxSize =  evalBox.constraint.maxSize();
		if(value!=null  &&  value.length()>maxSize)
			throw new FailedValidationException("The size of "+evalBox.fieldName+" is not allowed to be greater than :: "+maxSize);
	}
	private void minSize(EvalBox evalBox) throws FailedValidationException{
		String value = (String)  evalBox.content;
		int minSize =  evalBox.constraint.minSize();
		if(value!=null  &&  value.length()<minSize)
			throw new FailedValidationException("The size of "+evalBox.fieldName+" is not allowed to be lower than :: "+minSize);
	}
	private void blank(EvalBox evalBox) throws FailedValidationException{
		String value = (String)  evalBox.content;
		if(value!=null  &&  value.isEmpty() && !evalBox.constraint.blank())
			throw new FailedValidationException("The value of "+evalBox.fieldName+" is not allowed to be empty");
	}
	private void nullable(EvalBox evalBox) throws FailedValidationException{
		if(evalBox.content==null && !evalBox.constraint.nullable())
			throw new FailedValidationException("The value of "+evalBox.fieldName+" is not allowed to be null");
	}
	private void maxValue(EvalBox evalBox) throws FailedValidationException{
		Double maxValue = evalBox.constraint.maxValue();
		Double value = Double.valueOf(evalBox.content.toString());
		if(value>maxValue)
			throw new FailedValidationException("The value of "+evalBox.fieldName+" is greater than the maximum accepted");
	}
	private void minValue(EvalBox evalBox) throws FailedValidationException{
		Double minValue = evalBox.constraint.minValue();
		Double value = Double.valueOf(evalBox.content.toString()) ;
		if(value<minValue)
			throw new FailedValidationException("The value of "+evalBox.fieldName+" is lower than the minimum accepted");
	}
